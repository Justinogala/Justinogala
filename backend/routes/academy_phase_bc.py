...
BADGE_DEFINITIONS = [
    ...
    {"id": "capstone_complete", "title": "Builder", "description": "Complete a capstone project", "icon": "hammer", "color": "#F97316", "category": "achievement"},
...
async def check_and_award_badges(user=Depends(get_current_user)):
    ...
    capstones = await db.academy_capstone_submissions.count_documents({"user_id": user_id, "status": "completed"})
...
async def get_enhanced_dashboard(user=Depends(get_current_user)):
    ...
    capstone_count = await db.academy_capstone_submissions.count_documents({"user_id": user_id})
    ...
    return {
        ...
        "stats": {
            ...
            "capstones_completed": capstone_count,
        }
    }
...
@router.get("/capstone-projects")
async def list_capstone_projects(user=Depends(get_current_user)):
    """List user's capstone project submissions"""
    submissions = await db.academy_capstone_submissions.find(
...
async def submit_capstone(submission: CapstoneSubmission, user=Depends(get_current_user)):
    ...
    doc = {
        ...
        "user_name": user.get("name", ""), "title": submission.title,
...
async def update_capstone(project_id: str, submission: CapstoneSubmission, user=Depends(get_current_user)):
    """Update a capstone project"""
    result = await db.academy_capstone_submissions.update_one(
        {"id": project_id, "user_id": user.get("id")},
        {"$set": {
            "title": submission.title, "description": submission.description,
            "builder_project_id": submission.builder_project_id,
            "repo_url": submission.repo_url, "demo_url": submission.demo_url,
    ...
    if result.matched_count == 0:
        raise HTTPException(status_code=404, detail="Project not found")
    updated = await db.academy_capstone_submissions.find_one({"id": project_id}, {"_id": 0})
