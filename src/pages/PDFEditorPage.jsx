    ...
    return (
      <PageTransition>
        <div className="max-w-5xl mx-auto p-6 space-y-6" data-testid="pdf-editor-empty">
          ...
          {/* Fill Template Modal */}
          {fillModal && (
            <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm" data-testid="fill-template-modal-overlay" onClick={() => { setFillModal(null); setFillValues({}); }}>
              <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-2xl w-full max-w-md mx-4 overflow-hidden" onClick={e => e.stopPropagation()} data-testid="fill-template-modal">
                <div className="px-6 pt-6 pb-4 border-b border-gray-100 dark:border-slate-700">
                  <div className="flex items-center gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      <h3 className="text-base font-semibold text-gray-900 dark:text-white truncate">{fillModal.name}</h3>
                      <p className="text-xs text-gray-400 mt-0.5">Fill in the details below to generate your document.</p>
                    </div>
                    <Button variant="ghost" size="sm" onClick={() => { setFillModal(null); setFillValues({}); }} data-testid="fill-modal-close-btn">
                      <X className="w-4 h-4" />
                    </Button>
                  </div>
                </div>
                <div className="px-6 py-4 space-y-3 max-h-[60vh] overflow-y-auto">
                  {fillModal.fields.map((field, i) => (
                    <div key={field}>
                      ...
                      <Input
                        ...
                        data-testid={`fill-field-${i}`}
                      />
                    </div>
                  ))}
                </div>
                <div className="px-6 pb-6 pt-2 flex gap-2 justify-end">
                  <Button variant="outline" onClick={() => { setFillModal(null); setFillValues({}); }}>Cancel</Button>
                  <Button
                    className="bg-violet-600 hover:bg-violet-700 text-white"
                    onClick={submitFillModal}
                    disabled={generatingTemplate === fillModal.id}
                    data-testid="fill-modal-generate-btn"
                  >
                    {generatingTemplate === fillModal.id ? <Loader2 className="w-4 h-4 mr-1 animate-spin" /> : <FileText className="w-4 h-4 mr-1" />}
                    Generate Document
                  </Button>
                </div>
              </div>
